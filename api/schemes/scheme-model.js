...
function find() { // EXERCISE A
  ...
  */
    return db('schemes as sc') //FROM schemes as sc
...
async function findById(scheme_id) { // EXERCISE B
  ...
  */
      const rows = await db('schemes as sc') // FROM schemes as sc
      .leftJoin('steps as st', 'sc.scheme_id', 'st.scheme_id') // LEFT JOIN steps as st ON sc.scheme_id = st.scheme_id
      .where('sc.scheme_id', scheme_id) //  WHERE sc.scheme_id = 1
      .select('st.*', 'sc.scheme_name', 'sc.scheme_id') // SELECT sc.scheme_name, st.*
...
async function findSteps(scheme_id) { // EXERCISE C
  ...
  */
      const rows = await db('schemes as sc') // FROM schemes as sc
      .leftJoin('steps as st', 'sc.scheme_id', 'st.scheme_id') //  lEFT JOIN steps as st ON sc.scheme_id = st.scheme_id
      .select('st.step_id', 'st.step_number', 'instructions', 'sc.scheme_name') // SELECT st.step_id, st.step_number, instructions, sc.scheme_name
      .where('sc.scheme_id', scheme_id) //  WHERE sc.scheme_id = 1
      .orderBy('st.step_number') // ORDER BY st.step_number ASC;
